...
| 店舗全体（登録済み端末） | 50回 / 60分 | 端末が乗っ取られた場合の最終防衛線 |
| 店舗全体（未登録端末＝移行中のみ） | 10回 / 60分 | 移行期間中の総当たりを抑える。登録済み端末とはカウンタを分離しており、\n攻撃を受けても正規端末は巻き添えにならない |
...
2. サーバーから「端末未登録」と返されたら **店舗コード入力画面**（新規）へ自動で遷移
...
- 緊急復旧コード（`BOOTSTRAP_PAIRING_CODE`）を設定する場合、その1本が漏れると端末登録が可能になる。
  オフラインで保管し、使用後は値を変更する運用にする。**ちょうど8文字**で、`ABCDEFGHJKMNPQRSTUVWXYZ23456789` の文字だけを使うこと
  （入力欄が8文字までのため、それより長い値を設定すると緊急時に入力できず復旧口が機能しない。
   8文字でも約8500億通り＋回数制限があるため強度は十分）
